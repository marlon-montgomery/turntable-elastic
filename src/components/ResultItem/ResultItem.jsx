...
export const ResultItem = ({
  ...
  let socialLinks = [
    ...
    },
    {
      ...
      icon: require("../../assets/images/organization.png"),
    },
  ];

  socialLinks = socialLinks.map((socialLink) => {
    const link = socialLink.link;
    const isValid = link !== "" && link !== null && link !== undefined && link !== "—";
    return {
      ...socialLink,
      isValid,
    };
                ...
                <div className="detail-section-sub-values">
                  {availableInfoSections.content === null
                    ...
                    : availableInfoSections.map((item, index) => (
                      <div key={index} className="detail-section-sub-value">
                        {item.title}: {item.content}
                      </div>
                    ))}
                  {console.log(organization_url)}
          ...
          <div className="profile-actions-left-section">
            <div className="profile-action social-links">
              {socialLinks.map((socialLink, index) => (
                <div
                  key={index}
                  className={`social-link ${!socialLink.isValid && "dim"}`}
                  onClick={
                    socialLink.isValid ? () => navigate(socialLink.link) : null
                  }
                >
